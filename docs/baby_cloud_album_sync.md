...
- 支持 HTTP 和 HTTPS。
- 外网地址和内网地址分别保存。
- WebDAV 端点支持 `auto` / `lan` / `external` 三种模式；默认 `auto`，也可手动固定走内网或外网。
- 当前网络看起来在局域网时，自动模式优先检测内网地址；内网不可用时再检测外网。
- 云相册检测必须以用户配置的 WebDAV 路径和亲宝宝根目录为准，不能偷懒只测主机根 `/`，也不要擅自回退到同主机的根地址，否则 AList / OpenList 一类服务容易误判 405。
- WebDAV 根目录不存在时应自动创建。
